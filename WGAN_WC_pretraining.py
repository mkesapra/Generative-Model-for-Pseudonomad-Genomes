...
# -*- coding: utf-8 -*-
"""
Created on Mon Aug  8 22:53:35 2022
...
## Pre training the model with fake data 
core_genes = pd.read_csv(r'persistent.txt', header=None)

df_No_core = df_new.copy()

cond = df_new['Gene'].isin(core_genes[0])

for k in range(0, df_new.shape[0]):
    if (cond.iloc[k]):
        df_No_core.iloc[k] = df_new.iloc[k].apply(lambda x: 0)


## Get the gene p/a values of the corresponding genomes - 10% fake data
genome_fake_values = []
for k in range(0, int(0.1*len(genome_labels))):
    genome_fake_values.append(df_No_core[genome_labels[k]].tolist())
    
## Getting ready to input the data to the model: conversion to an array   
genome_fake_data = np.array(genome_fake_values)
## Fake genome labels
fake_labels_for_training = ['']* int(0.1*len(genome_labels))


all_genome_data = np.concatenate((genome_data,genome_fake_data))
all_genome_labels = true_labels_for_training + fake_labels_for_training


## Shuffle all the data before train/test split
from sklearn.utils import shuffle
all_data, all_labels = shuffle(all_genome_data, all_genome_labels, random_state=42)
